...
PRIVILEGES = {
    ...
    "Бета Тестер":         {"card_cd": CARD_COOLDOWN - 900, "coin_mult": 2, "battle_cd": BATTLE_COOLDOWN - 300},
}
...
CARDS = [
    ("Noob", "Обычная", 10, 5, "https://www.image2url.com/r2/default/images/1788843915247-2cb6b91e-dce3-4c3d-bf2d-ab6acd8e880e.jpg"),
    ("Tuxedo Noob", "Необычная", 25, 5, "https://www.image2url.com/r2/default/images/1789448225397-03027f01-1b55-4328-bbad-8e87a3142d81.jpg"),
    ("Guest", "Редкая", 35, 15, "https://www.image2url.com/r2/default/images/1789448067700-ba187114-0fba-420a-9309-ae88dcabd518.jpg"),
    ...
    ("Upgraded Titan Noob", "Легендарная", 105, 35, "https://kommodo.ai/i/NzYJcX7EBYjwAgk4P7GS"),
    ...
    ("Upgraded Large Noob", "Эксклюзивный", 45, 100, "https://kommodo.ai/i/d0RkJ8Plkgzt3mV35rix"),
]
...
def migrate():
    ...
    if prev_ver != BOT_VERSION:
        # при смене версии бота сбрасываем только кулдауны, остальное сохраняется
        conn.execute("UPDATE users SET last_bonus=0, last_card=0")
        conn.commit()
...
def do_broadcast(message, target):
    ids = [r["chat_id"] for r in get_all_chats()] if target == "all" else [target]
    ok, fail = 0, 0
    for cid in ids:
        try:
            safe_call(bot.copy_message, cid, message.chat.id, message.message_id)
            ok += 1
        except Exception:
            fail += 1
        time.sleep(0.35)
...
def callback_handler(call):
    ...
    if data.startswith("case:buy:"):
        ...
        # АНИМАЦИЯ: прогресс 10%..100%, сообщение редактируется
        try:
            anim = safe_call(bot.send_message, chat_id,
                             f"🧿 <b>Открываем {esc(case['name'])}...</b>\n[░░░░░░░░░░] 0%")
            for pct in range(25, 101, 25):
                if anim:
                    filled = pct // 10
                    bar = "█" * filled + "░" * (10 - filled)
                    safe_edit(chat_id, anim.message_id,
                              f"🧿 <b>Открываем {esc(case['name'])}...</b>\n[{bar}] {pct}%")
                time.sleep(0.18)  # быстрая анимация
        except Exception:
            pass
        ...
        if leveled:
            text = text.replace("</b>", f"\n🔋 Новый уровень: <b>{new_level}</b>!</b>")
        safe_call(bot.send_photo, chat_id, url, caption=text,
...
def on_successful_payment(message):
    ...
    if not it or puid != uid:
        safe_call(bot.reply_to, message, "⚠️ Ошибка платежа: счёт не найден.")
        return
    u = get_user(uid)
    if not u:
        return
    ...
    safe_call(bot.reply_to, message,
              f"✅ <b>Оплата прошла!</b>\n"
              f"👑 Твоя привилегия: <b>{esc(it['name'])}</b>\n"
