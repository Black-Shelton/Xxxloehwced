...
def txt_profile(u):
    return (f"{esc(u['nickname'])}, Ваш Профиль:\n"
            "════════════════════════════\n"
            f"📱 Ник: {esc(u['nickname'])}\n"
            f"🪪 ID: {u['user_id']}\n"
...
def trade_room_text(room, uid):
    ...
    other = trade_other(room, uid)
    uo = get_user(other)
    oname = esc(uo["nickname"]) if uo else str(other)
    banner = ""
...
def callback_handler(call):
            ...
            for pct in range(10, 101, 10):
                if anim:
                    ...
                    safe_edit(chat_id, anim.message_id,
                              f"🧿 <b>Открываем {esc(case['name'])}...</b>\n[{bar}] {pct}%")
                time.sleep(0.25)  # быстрая анимация
        except Exception:
            pass
    ...
    if data.startswith("top:"):
        ...
        for i, r in enumerate(rows):
            lines.append(f"{medals[i]} {esc(r['nickname'])} — <b>{r['v']}</b>")
        kb = types.InlineKeyboardMarkup()
